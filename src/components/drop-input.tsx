...
function DropInput({ onChange, ...rest }: DropInputProps) {
  ...
  return (
    ...
    >
      {preview ? (
        <Preview src={preview} onChange={handleClick} onRemove={handleRemove} />
      ) : (
        <EmptyInput />
      )}
      <input
...
function Preview({ src, onChange, onRemove }: PreviewProps) {
  const handleChange = (e: Event) => {
    e.stopPropagation();
    onChange();
  };
    ...
    <>
      ...
      <div className={styles.previewActions}>
        <button
          ...
          className={[styles.previewButton, styles.previewButtonSecondary].join(
            " ",
          )}
        >
          Remove image
        </button>
        <button onClick={handleChange} className={styles.previewButton}>
          Change image
        </button>
      </div>
    </>
